...
Arduino Library for splitting a RTCM-Stream input byte by byte into seperated messages.
	...
	MAX_RTCM_BUFFFERSIZE could be defined outside of this library

GNU GPL v3
by hagre 2019
*/
#ifndef rtcmstreamsplitter_h
#define rtcmstreamsplitter_h

#ifndef MAX_RTCM_BUFFFERSIZE
	#define MAX_RTCM_BUFFFERSIZE 1030
#endif	
class RTCMStreamSplitter
{
private:
    bool _inLine;
    uint8_t _inputStreamHash [3];
    uint8_t _receiveStream[MAX_RTCM_BUFFFERSIZE];
    uint16_t _receiveStreamLength;
    int16_t _receiveStreamLengthCtr;
    void hashData(uint16_t startctr,uint16_t lengthctr);

    const uint32_t crc24q[256] = {
...
public:
    RTCMStreamSplitter();
    uint16_t inputByte(uint8_t input);
    uint8_t outputStream[MAX_RTCM_BUFFFERSIZE];
    uint16_t outputStreamLength;
};
